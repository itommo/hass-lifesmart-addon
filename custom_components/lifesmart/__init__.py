...
def _available_platforms() -> List[str]:
    ...
    for p in POTENTIAL_PLATFORMS:
        if importlib.util.find_spec(f"custom_components.lifesmart.{p}") is not None:
            present.append(p)
        else:
            _LOGGER.debug("LifeSmart: skipping platform '%s' (module not found)", p)
    return present
    ...
    for setter in ("add_message_callback", "add_listener", "on_message", "set_message_handler"):
        if hasattr(client, setter):
            try:
                ...
                setattr(client, "_lifesmart_ws_cb", _on_message); bound = True
                _LOGGER.debug("LifeSmart: websocket listener attached via %s()", setter); break
            except Exception as exc:
